    ...
    const columns = useMemo(() => [
        ...
        { 
            ...
            render: (row) => {
                ...
                const excedenteGenerado = parseFloat(row.excedente) || 0; 
                ...
                return (
                    <div className="flex flex-col min-w-[120px]">
                        ...
                        {/* 2. EL DESGLOSE (ABONOS Y EXCEDENTES) */}
                        <div className="flex flex-wrap gap-1 mt-1">
                            {/* Plata a favor que venía de la cuota anterior */}
                            {excedenteAnterior > 0 && (
                            ...
                            {/* Plata que se abonó físicamente a ESTA cuota */}
                            {pagoTotal > 0 && (
                            ...
                            {/* Plata que sobró de esta cuota y se va pa la siguiente */}
                            {excedenteGenerado > 0 && (
                                <span className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[9px] font-black bg-emerald-50 text-emerald-700 border border-emerald-200 uppercase tracking-tighter">
                                    Sobra a favor: S/ {excedenteGenerado.toFixed(2)}
                                </span>
                            )}
                        </div>
                    </div>
        ...
        { 
            ...
            render: (row, _col, allRows) => {
                // Bloquea si alguna cuota anterior NO está pagada (estado 2)
                ...
                if (row.estado === 2) {
                    return (
                        <div className="flex justify-end pr-4">
                            <div className="w-6 h-6 bg-green-100 text-green-600 rounded-full flex items-center justify-center shadow-sm border border-green-200">
                ...
                return (
                    <div className="flex justify-end">
                        {bloqueada ? (
                            <div className="flex items-center gap-1 text-slate-300 pr-2">
                            ...
                            <button 
                                ...
                                className="bg-slate-900 hover:bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all shadow-lg hover:shadow-blue-200 active:scale-95 flex items-center gap-2"
                            >
                                {row.estado === 5 
                                    ? <><span>⏳</span> Revisando</>
                                    : <><CloudArrowUpIcon className="w-3.5 h-3.5" /> Pagar S/ {parseFloat(row.saldo_pendiente).toFixed(2)}</>
                                }
                            </button>
                        )}
                    </div>
