            ...
            <div className="relative space-y-5 p-1">
                ...
                {loading && (
                    <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center gap-3 rounded-2xl">
                        ...
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Aplicando reducción...</p>
                    </div>
                )}
                ...
                <div className="bg-slate-900 rounded-[24px] p-5 text-white">
                    ...
                    <p className="text-[9px] text-slate-400 font-bold mt-2">Cuota N° {cuota?.nro} — {cuota?.vencimiento}</p>
                </div>
                        ...
                        {PORCENTAJES_RAPIDOS.map((p) => (
                            <button key={p} type="button" disabled={loading}
                                ...
                                className={`px-3 py-1.5 rounded-xl text-[10px] font-black border-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed
                                    ${porcentaje === String(p) ? 'border-brand-red bg-brand-red text-white' : 'border-slate-200 text-slate-500 hover:border-brand-red/50'}`}>
                                {p}%
                            </button>
                        ))}
                    </div>
                ...
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Porcentaje a reducir (1–100) *</label>
                    <div className="relative">
                        <input type="number" min="1" max="100" value={porcentaje} disabled={loading}
                            ...
                            placeholder="Ej: 50"
                            className="w-full p-4 pr-10 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-black text-slate-800 focus:border-brand-red focus:bg-white outline-none transition-all disabled:opacity-50 disabled:cursor-not-allowed" />
                ...
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Motivo (opcional)</label>
                    <textarea value={motivo} onChange={e => setMotivo(e.target.value)} disabled={loading}
                        ...
                        className="w-full p-3 bg-slate-50 border-2 border-slate-100 rounded-2xl text-xs font-bold text-slate-700 focus:border-brand-red focus:bg-white outline-none transition-all resize-none disabled:opacity-50 disabled:cursor-not-allowed" />
                </div>
                ...
                <button onClick={() => handleSubmit(cuota?.id, moraCargo)} disabled={loading || !puedeSubmit}
                    ...
                    {loading ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <ScissorsIcon className="w-4 h-4" />}
