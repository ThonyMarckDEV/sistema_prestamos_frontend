...
const CambiarPresidenteModal = ({ isOpen, onClose, prestamo, onSuccess }) => {
    ...
    return (
        <ViewModal isOpen={isOpen} onClose={onClose}  hideFooter={true} title="Cambiar Presidente del Grupo" size="md">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-black text-slate-800 uppercase">
                            {presidenteActual?.nombre ?? 'Sin presidente'}
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        {candidatos.length === 0 ? (
                            <p className="text-xs text-red-500 font-bold">
                                No hay integrantes disponibles para asumir la presidencia.
                            </p>
                        ) : (
                            <div className="space-y-2">
                                {candidatos.map(int => (
                                    <div key={int.id}
                                        onClick={() => setNuevoPresidenteId(int.id)}
                                        className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all
                                            ${nuevoPresidenteId === int.id
                                                ? 'border-brand-red bg-brand-red-light/30'
                                                : 'border-slate-100 hover:border-brand-red/30 bg-white'}`}
                                    >
                                        ...
                                        <span className="ml-auto text-xs font-black text-brand-red">
                                            S/ {int.monto}
                                        </span>
                                    </div>
                                ))}
                    ...
                    <div className="pt-2 flex justify-end gap-3">
                        ...
                        <button type="submit" disabled={submitDisabled}
                            ...
                            {loading ? 'Procesando...' : <><StarIcon className="w-4 h-4" /> Confirmar Cambio</>}
                        </button>
                    </div>
                </form>
            </div>
