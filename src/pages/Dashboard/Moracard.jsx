...
const fmt = n =>
    parseFloat(n || 0).toLocaleString('es-PE', {
        minimumFractionDigits: 2
    });

const fmtN = n =>
    parseInt(n || 0).toLocaleString('es-PE');

const Chevron = ({ collapsed }) => (
    <div
        className={`w-6 h-6 flex items-center justify-center text-slate-400 flex-shrink-0 transition-transform duration-300 ${
    ...
    <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[480px]">
                            ...
                            <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                    <div className="w-7 h-7 rounded-lg bg-brand-red-light flex items-center justify-center flex-shrink-0">
                                        <span className="text-[9px] font-black text-brand-red">
                                            {f.abrev}
                                        </span>
                                    </div>
            ...
            <tfoot className="bg-slate-900 text-white">
                <tr>
                    <td className="px-4 py-3 text-[10px] font-black uppercase tracking-widest">
                        TOTAL
                    </td>

                    <td className="px-4 py-3 text-right text-sm font-black">
                        {esMonto
                            ? `S/ ${fmt(totales.saldo_inicial)}`
                            : fmtN(totales.saldo_inicial)}
                    </td>

                    <td className="px-4 py-3 text-right text-sm font-black">
                        {esMonto
                            ? `S/ ${fmt(totales.saldo_actual)}`
                            : fmtN(totales.saldo_actual)}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right text-sm font-black text-brand-gold">
                        ...
                        {esMonto
                            ? `S/ ${fmt(totales.variacion)}`
                            : fmtN(totales.variacion)}
                    </td>
                </tr>
            </tfoot>
        </table>
                ...
                >
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                            Seguimiento por asesor — saldo inicial vs actual
                        </p>
