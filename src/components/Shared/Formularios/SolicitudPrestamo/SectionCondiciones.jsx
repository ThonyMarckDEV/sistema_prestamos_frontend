...
const SectionCondiciones = ({ data, handleChange, isBlocked, calc }) => (
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
        <h3 className="text-sm font-black text-slate-700 uppercase mb-4 flex items-center gap-2">
            <BanknotesIcon className="w-5 h-5 text-red-600" /> Condiciones del Préstamo
        </h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10">
            <div>
                ...
                <input 
                    ...
                    className={`w-full p-2.5 border rounded-lg text-sm font-black outline-none disabled:cursor-not-allowed ${data.es_grupal ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-slate-50 border-slate-200 text-slate-900 focus:ring-1 focus:ring-blue-500'}`} 
                />
            </div>
            <div>
                ...
                <input 
                    ...
                    className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm font-black outline-none focus:ring-1 focus:ring-blue-500 disabled:cursor-not-allowed" 
                />
            </div>
            <div>
            ...
            <div>
                ...
                <select disabled={isBlocked} value={data.frecuencia} onChange={e => handleChange('frecuencia', e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm font-black outline-none focus:ring-1 focus:ring-blue-500 disabled:cursor-not-allowed">
        ...
        {/* WIDGET CALCULADORA */}
        {calc.montoBase > 0 && (
            <div className="mt-6 bg-slate-800 rounded-xl p-4 flex flex-col md:flex-row justify-between items-center gap-4 shadow-inner relative z-10">
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <div className="p-2 bg-blue-500/20 rounded-lg"><CalculatorIcon className="w-6 h-6 text-blue-400" /></div>
                    ...
                    <div><p className="text-[10px] text-slate-400 uppercase font-bold">Interés ({calc.porcentajeInteres}%)</p><p className="text-sm font-black text-red-400">S/ {calc.interesGenerado.toFixed(2)}</p></div>
                </div>
