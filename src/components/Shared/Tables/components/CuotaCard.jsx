...
export const CuotaCard = ({ cuota, i, cronograma, esVistaIntegrante, onHistorialModal, extraColumns }) => {
    ...
    const borderColor = d.esCancelada ? 'border-l-slate-300' : d.esRefinanciada ? 'border-l-blue-400' : d.saldo <= 0 ? 'border-l-green-400' : d.diasAtraso > 0 ? 'border-l-brand-red' : 'border-l-slate-200';
                ...
                <div className="px-4 pb-4 pt-1 bg-slate-50/60 border-t border-slate-100 space-y-0">
                    ...
                    {d.tieneExcedente && !d.esInactiva && (
                        <CardRow label="Excedente">
                            <ExcedenteContent
                                excAnterior={d.excAnterior} excAplicado={d.excAplicado} excConsumido={d.excConsumido} excGenerado={d.excGenerado}
                                label={esVistaIntegrante ? 'Exc. propio' : 'Excedente'}
                            />
                        </CardRow>
                    )}
                    {hayExcedentesIntegrantes && (
                        <CardRow label="Exc. integrantes">
